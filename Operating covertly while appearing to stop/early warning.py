...
threshold_value_up = max_value_second_column * 0.4  # 10% 的阈值
threshold_value_down = max_value_second_column * 0.4  # 10% 的阈值
...
for idx, probabilities in enumerate(final_probabilities_all):
    if probabilities is not None:
        # 检查最终概率条件 (假设类别索引从0开始)
        probability_condition = sum(probabilities[[0, 3]]) < sum(probabilities[[1, 2]])
