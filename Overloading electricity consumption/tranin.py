...
selected_clusters_per_round = [[3, 4], [5], [0, 1], [2, 3]]
...
for l in range(4):
    ...
    # # 加载已保存的SOM模型
    with open(f'som_model-chaofuhe_{l}.pkl', 'rb') as model_file:
...
for l in range(4):
    ...
    som_output_indices = np.array([som.winner(x) for x in subset])
    # 保存模型到文件
    # with open(f"som_model-chaofuhe_{l}.pkl", 'wb') as model_file:
    #     pickle.dump(som, model_file)
    # # 加载已保存的SOM模型
    with open(f'som_model-chaofuhe_{l}.pkl', 'rb') as model_file:
        som = pickle.load(model_file)
    ...
    fig, axs = plt.subplots(2, 5, figsize=(8, 6), dpi=200)
    ...
    ax_all_clusters = fig.add_subplot(2, 5, final_clusters + 1)
...
for l in range(4):
    with open(f'som_model-chaofuhe_{l}.pkl', 'rb') as model_file:
        ...
        soms.append(som)
# 定义选择的聚类组
selected_clusters_per_round = [[3, 4], [5], [0, 1], [2, 3]] #[[4], [5], [0,1], [3]]
# 初始化计数器
...
# 绘制聚类结果
fig, axs = plt.subplots(1, 4, figsize=(12, 12), dpi=200)
axs = axs.flatten()
...
for i in range(len(final_clusters)):
    ...
    max_value = aggregated_data['elec_consumption'].max()
    threshold = max_value * 0.8  # 10% 的阈值
    if cluster_label in [0, 2, 3]:  # 只标记第1、2、3类
            ...
            if value1 < value2:
                ...
                for row in range(2, ws.max_row + 1):  # 从第2行开始遍历（假设第1行为表头）
                    ...
                    if isinstance(next_day, pd.Timestamp):
                        cell_date = pd.Timestamp(next_day)
...
ave = [[[0.0] * n_clusters for _ in range(max_clusters)] for _ in range(n_clusters)]
