...
filtered_data = data.loc[data['ent_status'] == '正常开工'] # 过滤第 C 列为"责令停产"的数据
...
threshold_value_up = max_value_second_column * 0.7  # 10% 的阈值
threshold_value_down = max_value_second_column * 0.7  # 10% 的阈值
...
for idx, probabilities in enumerate(final_probabilities_all):
    if probabilities is not None:
        # 检查最终概率条件 (假设类别索引从0开始)
        probability_condition = sum(probabilities[[1]]) < sum(probabilities[[0, 2, 3]])
        ...
        if probability_condition:
            ...
            second_column_condition = data_normalized[idx, 1] > threshold_value_up
            ...
            if second_column_condition and second_column_condition_2:
