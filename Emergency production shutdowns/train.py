...
filtered_data = data.loc[data['ent_status'] == 'Normal production state'] # 过滤第 C 列为"责令停产"的数据
...
selected_clusters_per_round = [[0, 7, 8], [1, 5, 6, 7], [3, 9], [2, 7]]
...
for l in range(4):
    ...
    # # 加载已保存的SOM模型
    with open(f'som_model-tingche_{l}.pkl', 'rb') as model_file:
...
for l in range(4):
    ...
    max_clusters = 11
    T = fcluster(Z, max_clusters, criterion='maxclust')
    # final_clusters = 11 # 根据需要选择最终聚类结果，例如，选择前 k 个聚类
    ...
    som_index_mapping = {0: (0, 0), 1: (0, 1), 2: (0, 2), 3: (0, 3), 4: (0, 4), 5: (0, 5), 6: (0, 6), 7: (1, 0), 8: (1, 1), 9: (1, 2), 10: (1, 3), 11: (1, 4), 12: (1, 5), 13: (1, 6), 14: (2, 0), 15: (2, 1), 16: (2, 2), 17: (2, 3), 18: (2, 4), 19: (2, 5), 20: (2, 6),}
    agnes_indices = {}
    ...
    fig, axs = plt.subplots(2, 6, figsize=(8, 6), dpi=200)
    # 将axs展平以便于遍历
    axs = axs.flatten()
    ...
    ax_all_clusters = fig.add_subplot(2, 6, final_clusters + 1)
    lines = []
    labels = []
...
for l in range(4):
    ...
    som_index_mapping = {0: (0, 0), 1: (0, 1), 2: (0, 2), 3: (0, 3), 4: (0, 4), 5: (0, 5), 6: (0, 6), 7: (1, 0),
                         8: (1, 1), 9: (1, 2), 10: (1, 3), 11: (1, 4), 12: (1, 5), 13: (1, 6), 14: (2, 0), 15: (2, 1),
                         16: (2, 2), 17: (2, 3), 18: (2, 4), 19: (2, 5), 20: (2, 6), }
    agnes_indices = {}
    for cluster_id, som_indices in enumerate(cluster_indices):
        agnes_indices[cluster_id] = []
        for som_index in som_indices:
...
# 绘制聚类结果
fig, axs = plt.subplots(1, 4, figsize=(15, 12), dpi=200)
axs = axs.flatten()
...
for i in range(len(final_clusters)):
    ...
    if cluster_label in [1, 2]:  # 只标记第1、2、3类
        ...
        value1 = sample_data[2]  # 假设 value1 在 new_data 的第三列
        value2 = sample_data[3]  # 假设 value2 在 new_data 的第四列
        data_time = sample_data[1]  # 假设这是 pandas Timestamp 对象
        ent_name = sample_data[0]  # 企业名称
        data_time = pd.to_datetime(data_time)
        if value1 > value2 and value2 < value1 * 0.5:
...
# 初始化存储概率的字典
probabilities = {}
selected_clusters_per_round = []
for l in range(4):
    # 定义该轮次的selected_clusters为所有簇
    selected_clusters = list(range(0, final_optimal_k ))
    selected_clusters_per_round.append(selected_clusters)
# 遍历每一轮次
for l in range(4):
    ...
    for cluster_id in cluster_ids_to_select:
        ...
        indices = agnes_indices_all_rounds[l].get(cluster_id, []) #agnes_indices_all_rounds
