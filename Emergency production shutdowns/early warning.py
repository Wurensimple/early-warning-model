...
for _, group in aggregated_data.groupby('meter_no_tm'):  # 先按测量点名称分组
    for i in range(len(group) - 1):
        ...
        #     count += 1
            ...
            # 计算不连续日数并更新字典
            days_gap = (middle_date - first_date).days
            if days_gap > 1:
...
threshold_value_down = max_value_second_column * 0.13  # 10% 的阈值
...
for idx, probabilities in enumerate(final_probabilities_all):
    if probabilities is not None:
        # 检查最终概率条件 (假设类别索引从0开始)
        probability_condition = sum(probabilities[[0]]) < sum(probabilities[[1, 2, 3]])
        ...
        # 如果两个条件都满足，则记录该行
        if probability_condition and first_column_condition and second_column_condition:
            ...
            # rows_to_highlight.append(idx + 2 + count)  # +2 因为Excel行索引从1开始，而dataframe索引从0开始，并且考虑标题行
