...
dimension2 = 5
# 指定每一轮次选择的聚类组
selected_clusters_per_round = [[5], [0], [0], [4]] # 每一轮次选择的聚类组
soms = []
...
for l in range(4):
    ...
    # 将axs展平以便于遍历
    axs = axs.flatten()
    x_ticks = np.arange(0, 29, 5)  # 设置横坐标刻度从0到24，步长为1
    # 设置所有subplot的纵坐标范围
    for ax in axs:
        ax.set_ylim([0, +1])
    ...
    ax_all_clusters.set_xlim([0, 29])  # 设置横坐标范围
...
for l in range(4):
    with open(f'som_model-changqi_{l}.pkl', 'rb') as model_file:
        ...
        soms.append(som)
# 定义选择的聚类组
selected_clusters_per_round = [[5], [0], [0], [4]]
# 初始化计数器
...
for i in range(len(final_clusters)):
    ...
    if cluster_label in [1, 2]:  # 只标记第1、2、3类
        ...
        # 创建一个日期范围，包括 data_time 及其后四天
        date_range = pd.date_range(data_time, periods=30)
